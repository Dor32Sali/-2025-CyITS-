...
def validate_snapshot(
    ...
    light_state = snapshot.get("light_state", {})
    sensors = snapshot.get("sensor_readings", {})
    ...
    # -------------------------------------------------
    # 1. Basic range checks (like before)
    # -------------------------------------------------
    ...
    if expected_lights:
        ...
        if mismatches:
            add(
                ...
                severity="CRITICAL",
                attack=True,  # could be malicious tampering
            )

    # Extra case for your JSON: NS_GREEN but N/S are RED, E/W GREEN
    if phase == "NS_GREEN":
    ...
    # we treat it as possible attack (“edge black hole”).
    ...
    # Snapshot-based heuristic:
    #   - all directions red OR phase says ALL_RED / Is_All_Red=1
    #   - no emergency / preempt
    #   - queue is large -> suspect DoS on controller (forced all-red)
    ...
    # -------------------------------------------------
    # 6. Camera vs loop consistency (same as before, generic anomaly)
    # -------------------------------------------------
